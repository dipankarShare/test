#!/usr/bin/env python3
"""
Debug script for Chase credit card parsing to identify transaction extraction issues
"""
...
def debug_chase_parsing():
    ...
    # Try to find a Chase PDF in the data directory
    data_dir = "data"
    chase_pdfs = []
    
    if os.path.exists(data_dir):
        for file in os.listdir(data_dir):
            if file.lower().endswith('.pdf') and 'chase' in file.lower():
                chase_pdfs.append(os.path.join(data_dir, file))
    
    if not chase_pdfs:
        print("❌ No Chase PDF files found in data directory")
        print("Please place a Chase credit card statement PDF in the data/ directory")
        return
    
    pdf_path = chase_pdfs[0]
    ...
    print(f"\n🔍 Testing alternative transaction patterns...")
    
    # Pattern 1: Date Description Amount
    alt_pattern1 = r"(\d{1,2}[/-]\d{1,2})\s+([^$]+?)\s+([-]?\$?[\d,]+\.?\d*)"
    print(f"Alternative pattern 1: {alt_pattern1}")
    
    for i, line in enumerate(transaction_lines[:5]):
        match = re.search(alt_pattern1, line)
        if match:
            print(f"✅ Line {i+1} MATCHES alt pattern 1: {line}")
            print(f"  Groups: {match.groups()}")
        else:
            print(f"❌ Line {i+1} NO MATCH alt pattern 1: {line}")
    
    # Pattern 2: Date Description Amount (more flexible)
    alt_pattern2 = r"(\d{1,2}[/-]\d{1,2})\s+(.+?)\s+([-]?\$?[\d,]+\.?\d*)"
    print(f"Alternative pattern 2: {alt_pattern2}")
    
    for i, line in enumerate(transaction_lines[:5]):
        match = re.search(alt_pattern2, line)
        if match:
            print(f"✅ Line {i+1} MATCHES alt pattern 2: {line}")
            print(f"  Groups: {match.groups()}")
        else:
            print(f"❌ Line {i+1} NO MATCH alt pattern 2: {line}")
